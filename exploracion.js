...
function createSpaceship() {
    ...
    loader.load('/constelacion/nave.obj', (obj) => {
        ...
        console.log('OBJ no disponible, usando modelo fallback');
...
function createFallbackSpaceship() {
    ...
    const glowMaterial = new THREE.MeshBasicMaterial({ color: 0x00ffff, transparent: true, opacity: 0.8 });
...
function createPlanet(name, data) {
    ...
    const speedFactor = state.realisticMode ? (365 / data.orbitalPeriod) : data.speed;
    
    planets[name] = {
        mesh,
        orbit: data.distance,
        angle: Math.random() * Math.PI * 2,
        speed: speedFactor * 0.001,
        rotationSpeed: 0.01
    };
}
...
function createMoon(name, data) {
    ...
    const speedFactor = state.realisticMode ? (27.3 / data.orbitalPeriod) : data.speed;
...
function updatePlanets() {
    const timeMultiplier = state.realisticMode ? state.timeScale : state.timeScale;
    state.realTimeElapsed += (16.67 * state.timeScale);
...
function updateUI() {
    ...
    gameClock.textContent = formatTime(state.realTimeElapsed);
